...
public class LargeBackpackItem extends Item {
    ...
    public static ItemStackHandler getInventory(ItemStack backpackStack, Level level) {
        ItemStackHandler handler = new ItemStackHandler(LargeBackpackMenu.BACKPACK_SIZE);
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(@NotNull Level level, @NotNull Player player, @NotNull InteractionHand hand) {
        ...
        if (!level.isClientSide && player instanceof ServerPlayer serverPlayer) {
            if (stack.get(ModDataComponents.BACKPACK_INVENTORY_NBT) == null) {
                ItemStackHandler newHandler = new ItemStackHandler(LargeBackpackMenu.BACKPACK_SIZE);
                saveInventory(stack, newHandler, level); // Pasar 'level'
            }

            serverPlayer.openMenu(new SimpleMenuProvider(
                    (id, playerInventory, p) -> new LargeBackpackMenu(id, playerInventory, stack), // BackpackMenu necesitará el level también
                    Component.translatable("container.supermodreborn.large_backpack")
            ));
        }
